...
'''
    --------------------说明----------------------
        ...
        任务本所需的数据上节课已经增加到数据库中，
        所以此文件这节课不需要运行，
        仅作为备用文件，防止有的孩子数据库中没有数据。
...
todo8 = {'_id': str(uuid.uuid1()), 'username': '小猿', 'state': '已完成', 'subject': '英语', 'content': '做6篇阅读理解', 'date': '2021-04-14'}
...
# 练习：使用函数todo_insert()，将任务添加到集合c_todo中
todo_insert(todo1)
todo_insert(todo2)
todo_insert(todo3)
todo_insert(todo4)
